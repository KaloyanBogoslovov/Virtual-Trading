...
public class NewPosition {
  ...
  public NewPosition(Set<String> symbols) {
  ...
  private boolean symbolAndVolumeInputNotEmpty() {
    return !symbolTF.getText().equals("") && !volumeTF.getText().equals("")
        && volumeTF.getText().matches("\\d+");
  }
  ...
  private void saveTransaction(String type) throws SQLException {
    ...
    BigDecimal margin = volume.multiply(purchaseStockPrice).divide(leverage); // devided by the
                                                                              // leverage
