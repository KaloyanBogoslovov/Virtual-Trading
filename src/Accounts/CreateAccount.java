...
public class CreateAccount {
	...
	public  CreateAccount()  {
		...
		leverageCB.getItems().addAll("1:3","1:4","1:5","1:6","1:7","1:8","1:10","1:20","1:30","1:40","1:50","1:100","1:200");
		...
		grid.getChildren().addAll(firstNameLabel, firstNameTF, lastNameLabel, lastNameTF, userNameLabel,userNameTF,passLabel,passTF,balanceLabel,balanceTF,leverageLabel,leverageCB,exchangeLabel,exchangeCB,hbox,emptyField);
	...
	private void addNewAccToDB() throws SQLException, ClassNotFoundException{
		...
		String data = "INSERT INTO USERS " +
	"VALUES("
	+"'"+userNameTF.getText()	+"'"+", "
	+"'"+firstNameTF.getText()	+"'"+", "
	+"'"+lastNameTF.getText()	+"'"+", "
	+"'"+passTF.getText()		+"'"+", "
	+ Long.parseLong(balanceTF.getText())
	+", "+leverage()+", "+0+", "+0+", "
	+Long.parseLong(balanceTF.getText())+", "
	+Long.parseLong(balanceTF.getText())
			+", "+0+","+"'"+stockExchange()+"'"+")";
...
//		}
	...
	private void checkForEmptyFields(){
		if(!firstNameTF.getText().equals("")&&!lastNameTF.getText().equals("")&&!balanceTF.getText().equals("")&&!userNameTF.getText().equals("")&&!passTF.getText().equals("")){
			i+=2;
			emptyField.setText("");
			}else{
	...
	private void checkIfUsernameIsFree() throws ClassNotFoundException, SQLException{
		...
		while(usernames.next()){
			if(userNameTF.getText().equalsIgnoreCase(usernames.getString(1))){i--;
	...
	private void checkLastName(){
		if(lastNameTF.getText().matches("[a-zA-Z]+")){i++;
	...
	private void checkBalance(){
		if(balanceTF.getText().matches("\\d+")){i++;
	...
	private String stockExchange(){
		...
		case"NYSE,NASDAQ":return exchange = "NYSE,NASDAQ";
		case"Frankfurt SE":return exchange = "FRANKFURT";
		}
		return exchange;
	}
}
