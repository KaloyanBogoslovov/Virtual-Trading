...
public class ChartDataFromYahoo extends YahooFinance{
	...
	public String makeURL(){
		...
		String fromTo="&a="+fr+"&b="+from.charAt(3)+from.charAt(4)+"&c="
		+from.charAt(6)+from.charAt(7)+from.charAt(8)+from.charAt(9)+"&d="
		+t+	"&e="+to.charAt(3)+to.charAt(4)+"&f="
		+to.charAt(6)+to.charAt(7)+to.charAt(8)+to.charAt(9)+"&g=";

        String url = "http://ichart.yahoo.com/table.csv?s="+company+fromTo+interval+"&ignore=.csv";
		return url;
	}
    ...
    private ChartData parseCSVAndSetChartData(String line) {
        ...
        return new ChartData(data[0],data[1],data[3],data[2],data[4],data[6],Long.parseLong(data[5]));
    }
